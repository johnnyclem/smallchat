...
- **Compaction integration.** `TruthAwareCompactor` decorates any `Compactor` so every compacted state carries an `## Asserted Truth (ledger)` section, rebuilt from the current selection on each round (truth is durable residue — it survives L1→L3 rather than compacting away). `CompactedState` gains an optional structured `truth` field. `truthToInvariantRecords` projects the selection into L4-shaped records with the confidence type riding along inside the value (`[TB]` vs `[UV — UNVERIFIED]`) — the two-axes-not-one contract, so an ingested UV can never be mistaken for a proven invariant in a string-typed store.
- **Proposal-only write path.** `proposeInvariants` drafts candidate invariants (settled configuration entities, live decisions) as `PROPOSAL(kind: 'uv')` JSONL lines with `signal.source: 'shorthand-compaction'`, deduplicated by `targetRef` across rounds via `appendProposalsFile`. There is no anonymous write path: generic identities (`system`, `assistant`, `agent`, …) are rejected at the schema level (`assertAccountableAuthor`), mirroring stenographer's authorship floor — the compactor cannot sign its own output.
- Everything is re-exported from `@smallchat/core`'s root entry point and available as the `@shorthand/core/truth` subpath. 17 new tests cover the round trip, lifecycle classification, contested pairing, displacement-on-sync, L4 projection, and anonymous-author rejection.
...
- **`LocalTransport` sandbox documentation and timeout correctness.** `sandbox.enabled` never isolated the registered handler's own capabilities (it's a plain JS closure with full `require`/`process`/`fs` access regardless of the `vm` context invoking it) — docs and a one-time runtime warning now say so plainly instead of implying otherwise. While verifying this, found and fixed a real bug: `timeoutMs` didn't actually bound a handler that hangs without ever resolving, in both the sandboxed path (`vm.Script`'s own `timeout` only bounds synchronous execution) and the shared `withTimeout()` helper (awaited the wrapped operation directly instead of racing it against the timer). Both now race against a timer so a non-cooperating handler can't hang the caller past its configured timeout.
- **Hardening in depth.** `SCDictionary.unwrap()` now builds its result via `Object.create(null)` instead of `{}`, closing a dormant prototype-pollution path if a `"__proto__"` key ever reaches it. `src/mcp/artifact.ts` now parses compiled-artifact and manifest JSON through `safeJsonParse` (previously bare `JSON.parse`), matching the guarantee already made for manifest/config loading elsewhere.

### Dependencies
- **Critical CVE resolved.** `vitest` bumped from `^3.0.0` to `^3.2.7`, closing GHSA-5xrq-8626-4rwp (arbitrary file read/execute via the Vitest UI server, affecting <3.2.6).
- **`npm audit fix` applied.** Pulled in patched `hono`/`@hono/node-server`/`fast-uri`/`ip-address`/`body-parser` (transitive via `@modelcontextprotocol/sdk`) and patched `next`/`postcss`/`sharp`/`vite`/`esbuild`/`nanoid` in `packages/nextjs`, resolving 11 of 13 previously-reported high/moderate findings without any `package.json` range changes. The remaining finding (`adm-zip` <0.6.0 via `onnxruntime-node`) is accepted risk: it's invoked only by `onnxruntime-node`'s install-time postinstall script to unpack its own npm-hosted prebuilt binary, never on attacker-reachable input, and no `onnxruntime-node` release yet depends on `adm-zip >=0.6.0`.
- **Not taken:** bumping `commander` (13→15) and `better-sqlite3` (11→13) — both now require Node >=22, which would silently raise this project's effective minimum Node version past its declared `engines.node: >=20.0.0`. Left as a deliberate follow-up requiring an explicit Node-floor decision.
- **`packages/docs` (44 → 22 vulnerabilities).** This package carries its own `package-lock.json` outside the npm workspace, so it wasn't covered by the root-level fixes above — confirmed by GitHub reporting 106 total repo vulnerabilities on push against the 13 `npm audit` found at root. `npm audit fix` (no `--force`, no `package.json` changes) resolved both critical findings and 22 highs there. The remaining 22 need a coordinated `@docusaurus/*` 3.6→3.10 upgrade, deferred for the same reason as the commander/better-sqlite3 bumps above.

### Fixed — docs site build was broken
- **`docs/integrations/index.md` linked to LoomMCP's page with a relative `./loom-mcp`**, which the page's `slug: /integrations` frontmatter override caused Docusaurus to resolve to the wrong path (`/docs/loom-mcp` instead of `/docs/integrations/loom-mcp`), failing `docusaurus build`'s broken-link check. Nothing in CI previously built the docs site, so this had been silently broken. Fixed to an absolute path; `packages/docs`'s CI job (below) now builds it on every change.

### CI
- **Added `.github/workflows/ci.yml`.** Nothing previously gated merges despite `test`/`lint`/`build` npm scripts all existing. Runs lint + test + build + build:packages on a Node 20/22 matrix, a separate `npm audit --audit-level=critical` job, and a `packages/docs` job (its own install, build, and audit, since it's outside the root workspace).

### Fixed — Selector table pollution (production report from HyperVault)
- **Resolved intents no longer surface as phantom tools.** `SelectorTable.resolve()` — the runtime path that interns a natural-language intent — shared its interning map and vector index with compiled tool selectors. Once a process resolved any intent, that intent showed up as a fake "tool" in `selectorTable.all()` (no owning `ToolClass`, unexecutable) and, worse, `refine()`'s "did you mean?" search could match a repeated intent against *itself* at ~1.0 similarity, surfacing the user's own query as the top suggestion — and a caller dispatching that suggestion's `selectorId` got a dead selector ("no longer in the toolkit"). Selectors now carry a `provenance: 'tool' | 'intent'` tag; `SelectorTable.all()` excludes intent selectors by default (opt in via `{ includeIntents: true }`), and a new `SelectorTable.searchTools()` filters intent selectors out of vector search results. Dispatch resolution, refinement, and the forwarding chain's broadened search all switched from raw `vectorIndex.search()` to `searchTools()`.
- **Bounded intent retention.** Every distinct intent a process resolves used to be retained in the table forever (unbounded memory growth + steadily more self-similar noise in later searches). Intent selectors are now LRU-evicted past a configurable cap (`RuntimeOptions.maxIntentEntries`, default 500); compiled tool selectors are never evicted.

### Added — Optional guard against auto-dispatch without an LLM client
- **`RuntimeOptions.requireLLMForSubHighDispatch`.** Without an `LLMClient`, MEDIUM-tier verification degrades to schema-only (an effective pass-through) and LOW-tier decomposition can't run at all — both tiers previously fell through to executing the best vector match unconditionally, auto-firing tools on as little as 0.60 confidence. That's fine for read-only tools and a footgun for destructive ones. Setting this flag makes MEDIUM/LOW resolutions defer to the refinement protocol (ask the user) instead of auto-dispatching when no `LLMClient` is configured. Off by default to preserve existing behavior; has no effect once an `LLMClient` is wired in.

...
### CI
- **Added `.github/workflows/ci.yml`.** Nothing previously gated merges despite `test`/`lint`/`build` npm scripts all existing. Now runs lint + test + build + build:packages on a Node 20/22 matrix, plus a separate `npm audit --audit-level=critical` job.
